...
def search_jobs(preferences_path: str = "data/preferences.yaml") -> list[dict]:
            ...
            try:
                ...
                for _, row in df.iterrows():
                    ...
                    if not url or url in seen_urls:
                        continue
                    description = _clean(str(row.get("description", "") or "").strip())
                    if not description:
                        skipped_empty += 1
                        continue
                    seen_urls.add(url)
                ...
                console.print(
                    f"  [green]'{role}' @ '{loc}': {len(df)} scraped → {added} added[/green]"
                )
            except Exception as e:
                console.print(
                    f"[yellow][Search] Warning: jobspy '{role}' @ '{loc}' failed — {e}[/yellow]"
                )
    ...
    if greenhouse_enabled:
        ...
        try:
            ...
            gh_jobs = fetch_greenhouse_jobs(
                companies.get("greenhouse", []), keywords=roles
            )
            before = len(all_jobs)
            for job in gh_jobs:
            ...
            console.print(
                f"  [green]Greenhouse: {len(gh_jobs)} fetched → {len(all_jobs) - before} added[/green]"
            )
        except Exception as e:
            console.print(
                f"[yellow][Search] Warning: Greenhouse fetch failed — {e}[/yellow]"
            )
    ...
    if lever_enabled:
        ...
        try:
            ...
            console.print(
                f"  [green]Lever: {len(lv_jobs)} fetched → {len(all_jobs) - before} added[/green]"
            )
        except Exception as e:
            console.print(
                f"[yellow][Search] Warning: Lever fetch failed — {e}[/yellow]"
            )

    if skipped_empty:
        console.print(
            f"[dim][Search] Skipped {skipped_empty} job(s) with empty descriptions.[/dim]"
        )
    ...
    console.print(f"[bold green]{len(all_jobs)} jobs found ({breakdown})[/bold green]")
